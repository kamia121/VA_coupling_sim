...
function trOut() {
  ...
  $('#out-tr').innerHTML = row([
    ...
    return `<p class="status">At peak systole, the RV–RA pressure difference is ${R.rec.Prv[i].toFixed(0)} − ${R.rec.Psv[i].toFixed(0)} = ${grad.toFixed(0)} mmHg, which equals 4 × ${v.toFixed(2)}², where ${v.toFixed(2)} m/s is the Doppler peak velocity.</p>`;
  })() : '');
  tapOut();
}
...
function cplOut() {
  ...
  ]) + `<p class="truth">The catheter values are an Ea of ${R.lv.Ea.toFixed(2)} and an Ees of ${R.lv.Ees.toFixed(2)} mmHg/mL, for an Ea/Ees of <b>${R.lv.EaEes.toFixed(2)}</b>. The normal values are 1.43, 2.30, and 0.62.</p>`;
  ...
  ]) + `<p class="truth">The catheter value of RV Ees/Ea is <b>${R.rv.EesEa.toFixed(2)}</b>${R.rv.EesEa < 0.805 ? ', which is below 0.805' : ''}. The normal value is 2.00.</p>`;
}
